...
"""Library of extra routines for data analysis"""
...
def find_bbfs(inp, thresh, direction):
    ...
    if len(times_d64) > 1 and any(np.diff(times_d64[:, 0]).astype(np.int64) > 60e9):
        idx = np.where(np.diff(times_d64[:, 0]).astype(np.int64) > 60e9)[0] + 1
    ...
    for t_ in times_clusters:
        idx = [
            np.where(inp.time.data == t_[0][0])[0][0],
            np.where(inp.time.data == t_[-1][1])[0][0],
        ]
        if direction.lower() == "earthward":
            if any(inp.data[: idx[0], 0] < 100.0):
                idx_left = np.where(inp.data[: idx[0], 0] < 100.0)[0][-1]
            else:
                idx_left = int(0)

            if any(inp.data[idx[-1] :, 0] < 100.0):
                idx_righ = np.where(inp.data[idx[-1] :, 0] < 100.0)[0][0]
                idx_righ += idx[-1]
            else:
                idx_righ = int(-1)
...
def fairfield(time, x, y, z):
    ...
    dipole_sm_ = ts_vec_xyz(
        r_gsm.time.data, np.tile([0, 0, 1], (len(r_gsm.time.data), 1))
    )
    dipole_gsm = cotrans(dipole_sm_, "sm>gsm")
    chi = np.arctan(dipole_gsm[:, 0] / dipole_gsm[:, 2])

    h_0, d, y_0 = [10.5 * 6371, 22.5 * 6371, 14 * 6371]
    delta_z = ((h_0 + d) * np.sqrt(1 - r_gsm.data[:, 1] ** 2 / y_0**2) - d) * np.sin(
        chi
    )
...
def _shue_mc(r_z_mc, alp_mc):
    ...
    with np.errstate(divide="ignore"):
        r_ = r_z_mc * (2.0 / (1 + np.cos(theta_))) ** alp_mc
...
def estimate_mp(days):
    ...
    b_z, d_p = [], []
    logging.info("Loading OMNI data..")
    ...
    r_z_mc = ufloat(fitdist_rz.getMean()[0], fitdist_rz.getStandardDeviation()[0])
    alp_mc = ufloat(fitdist_al.getMean()[0], fitdist_al.getStandardDeviation()[0])
    ...
    bnds_avg = np.vstack(
        [
            np.hstack([x_mp_avg, np.flip(x_mp_avg)]),
            np.hstack([y_mp_avg, np.flip(-y_mp_avg)]),
        ]
    )
    bnds_plu = np.vstack(
        [
            np.hstack([x_mp_plu, np.flip(x_mp_plu)]),
            np.hstack([y_mp_plu, np.flip(-y_mp_plu)]),
        ]
    )
    bnds_min = np.vstack(
        [
            np.hstack([x_mp_min, np.flip(x_mp_min)]),
...
def fit_df(b_gsm):
    ...
    b_smooth = ts_scalar(
        b_gsm.time.data, signal.savgol_filter(b_gsm.data[:, 2], 161, 5)
    )
    ...
    b_filt = ts_scalar(b_df.time.data, signal.savgol_filter(b_df.data[:, 2], 161, 5))

    b_z = b_filt
    p_, c_ = optimize.curve_fit(bz_model, np.linspace(-30, 15, len(b_z)), b_z)
    a, b, c = p_
